...
static double est_dist = 0.0, t_filter = 0.0, k1_gain = 0.2, temp1=0.0;
        ...
        static double error_tmp = 0.0, int_error = 0.0;
     ...
     static double spd, pre_v, v_tmp, v_t=0.0;
    ...
    // static double f_mode=0.0;
     ...
     static double pre_a_des, lead_a_des;

// from control
static double s_1=0.0, s_2=0.0, k1=0.0, k2=0.0, usyn=0.0,a,b;
...
//            }
           ...
           if( radar_fault == 1 )
             {
                error_tmp += radar_range - est_dist;
                alarm_t_filter += delta_t;
                if( alarm_t_filter > 1.0 )
                  {
                        if( fabs(error_tmp) > 50.0 ) 
                                radar_fault = 1;
                        else
                          {
                                radar_fault = 0;                                
     ...
     {                          
          ...
          ref_v=pre_v;
          ref_a=pre_a;
          //ref_a=0.7*pre_a+0.3*lead_a;
          //ref_a=pre_a_des;
          //ref_a=0.7*pre_a_des+0.3*lead_a_des;
     }
     else
     {
        ...
        if (comm_fault < 0.5)   
          {
            ...
            {
                ref_v=lead_v + ((v_t-5.0)/10.0)*(pre_v - lead_v);
                ref_a=lead_a + ((v_t-5.0)/10.0)*(pre_a - lead_a);
            }
            else
            {
                ref_v= pre_v; 
...
//         if (t_filter < 8.0)
//         {
//          k1 = 0.25 + 0.457*t_filter/8.0;
//          k2 = 0.5  + 0.207*t_filter/8.0;
//         }
//         else
//         {
//          k1=0.707;  
//          k2=0.707;
//         }  
        if (fabs(distance - temp_dist) > 5.0)
        {
         k1 = 0.35;
         k2 = 0.4;
        }
        else if (fabs(distance - temp_dist) > 0.5)
        {
         k1 = 0.25;
         k2 = 0.5 ;
        }
    ...
    if ( (pre_a < -0.5) || (pre_run_flag < 0.0) || (lead_run_flag < 0.0) )
    {
        if (decel_sw==0)
        {
            v0=v;
