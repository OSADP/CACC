...
#define Max_comm_loss   10
static const int N_st_pre=17;
static const int N_st_lead=17;

//#define Max_comm_count  255


static double subject_veh_id=98; 
static int control_pos_int=2;
...
//     }
   ...
   {                    
        ...
        else
        {
            ...
            pre_veh[6]=*(data_in[1][15]); //pre_a;  // PinPoint accel[55]; but JBus accel []; JBus a
            ...
            pre_veh[15]=(*(data_in[1][38]))/100; //pre_a_des
   ...
   { 
        ...
        else if  (control_pos_int==2)
        {
            ...
            pre_veh[4]=*(data_in[1][0]); //pre_drive_mode;
            pre_veh[5]=*(data_in[1][27]); //pre_man_des;
            pre_veh[6]=*(data_in[1][15]); //pre_a;  // PinPoint accel; but JBus accel not passed; a_des
            pre_veh[7]=*(data_in[1][49]); //pre_v;
            pre_veh[8]=*(data_in[1][35]); //pre_d;
            pre_veh[9]=*(data_in[1][37]); //pre_v_ref;
            pre_veh[10]=*(data_in[1][35]); //pre_d_ref;
            ...
            pre_veh[13]=*(data_in[1][57]); //pre_brk_pedal_deflect
            pre_veh[14]=*(data_in[1][37]); //pre_v_des
            pre_veh[15]=(*(data_in[1][38]))/100.0; //pre_a_des
            pre_veh[16]=*(data_in[1][66]); //pre_run_flag
            ...
            lead_veh[6]=*(data_in[1][15]); //lead_a;  // PinPoint accel; but JBus accel not passed; JBus a
            ...
            lead_veh[15]=(*(data_in[1][38]))/100.0; //lead_a_des 
        ...
        else // if (control_pos_int > 2)
        {
            ...
            pre_veh[6]=*(data_in[control_pos_int-1][15]); //pre_a;  // PinPoint accel[55]; but JBus accel not passed; JBus a
            ...
            pre_veh[15]=(*(data_in[control_pos_int-1][38]))/100.0; //pre_a_des
